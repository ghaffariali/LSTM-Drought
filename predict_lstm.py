...
LSTM method. The code uses five features and loops over different values of 
...
def get_traintest(p_normalized, t_normalized, n_normalized, e_normalized, s_normalized, 
    ...
    X_train = X[:-n_pred,:,:]; y_train = y_n[:-n_pred,:]
    X_test = X[-n_pred:,:,:]; y_test = y_n[-n_pred:,:]
    return X_train, y_train, X_test, y_test
...
for cell in p_normalized.index[m:]:
    ...
    with warnings.catch_warnings():
        ...
        results = Parallel(n_jobs=-1)\
            (delayed(predict_batchsize)(frame_b, b, epochs) for b in np.arange(1,9))
...
df_p.to_csv(os.path.join(output_dir, f"{var}\{n_steps}_{var}_{m}_{n}_{complexity}.csv"), index=None)   
np.save(os.path.join(output_dir, f"{var}\{n_steps}_metrics_{m}_{n}_{complexity}.npy"), metrics)
np.save(os.path.join(output_dir, f"{var}\{n_steps}_predictions_{m}_{n}_{complexity}.npy"), predictions)
# np.save(os.path.join(output_dir, f"{var}\{n_steps}_losses_{m}_{n}_{complexity}.npy"), losses)
end_time = time.time()
