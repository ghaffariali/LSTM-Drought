...
def predict_batchsize(frame_b, b, epochs, complexity):
    random.seed(42)
    np.random.seed(42)
    tf.random.set_seed(42)
...
n_steps=48; n_pred=12; n_features=5; var='SM'                                            # data preparation parameters
    ...
    for epochs in [50,100,150]:
        for complexity in ["low","moderate","high"]:
            ...
            # for cell in cells_east:
                print(f'\nStarted predicting for cell {cell} in {tag} with {complexity} complexity with epochs={epochs}:')
                X_train, y_train, X_test, y_test = get_traintest(
                ...
                with warnings.catch_warnings():
                    ...
                    results = Parallel(n_jobs=-1)\
                        (delayed(predict_batchsize)(frame_b, b, epochs, complexity) for b in tqdm(np.arange(1,17)))
            ...
            df_p.to_csv(os.path.join(output_dir, f"{var}\{tag}_e{epochs}_{n_steps}_{var}_{m}_{n}_ft_{complexity}.csv"), index=None)   
            np.save(os.path.join(output_dir, f"{var}\{tag}_e{epochs}_{n_steps}_metrics_ft_{complexity}.npy"), metrics)
            np.save(os.path.join(output_dir, f"{var}\{tag}_e{epochs}_{n_steps}_predictions_ft_{complexity}.npy"), predictions)
            np.save(os.path.join(output_dir, f"{var}\{tag}_e{epochs}_{n_steps}_losses_ft_{complexity}.npy"), losses)
